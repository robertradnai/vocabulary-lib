...
from tests.utils import reset_test_env, TEST_DICT_PATH
from vocabulary.alternatives import AlternativePoolBuilder
from vocabulary.dataaccess import load_wordlist_book, WordListDaoExcel
...
class TestFileMgr(unittest.TestCase):
    ...
    def test_vocabulary(self):
        ...
        word_list_dao = WordListDaoExcel(TEST_DICT_PATH)

        assert word_list_dao.is_word_list(word_list_name)
        word_list = word_list_dao.get_word_list(word_list_name)

        pool_builder = AlternativePoolBuilder()
        for word_list_name in word_list_dao.get_word_list_names():
            pool_builder.extend_pool(word_list_dao.get_word_list(word_list_name))
